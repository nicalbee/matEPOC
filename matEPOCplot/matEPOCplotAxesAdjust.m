function matEPOCplotAxesAdjust(handle,~)
% mepOSCCI3: mepPlotAxesAdjust
%
% mepPlotAxesAdjust(handle,event);
%
% notes:
% handles the plotting for x and y axes adjustments for the mepPlot
...
%         end
    case 'diff'
        ...
        if isnumeric(value)
            ...
            xlim(2) = xlim(1) + value;
            if xlim(2) > size(mep.tmp.data,1)*(1/mep.tmp.Hertz)
                xlim(2) = size(mep.tmp.data,1)*(1/mep.tmp.Hertz);
                xlim(1) = xlim(2) - str2double(get(diff_h,'string'));
            end
        elseif strcmp(value,'all')
            xlim = [1*(1/mep.tmp.Hertz) size(mep.tmp.data,1)*(1/mep.tmp.Hertz)];
            set(handle,'string',diff(xlim));
        end
    case 'upper'
        if value > xlim(1)
            if value < size(mep.tmp.data,1)*(1/mep.tmp.Hertz)
                xlim(2) = value;
                xlim(1) = xlim(2) - str2double(get(diff_h,'string'));
            else
    ...
    case 'lowerEnd'
        xlim(1) = 1*(1/mep.tmp.Hertz);
        xlim(2) = xlim(1) + str2double(get(diff_h,'string'));
        
    case 'down'
        % + overlap here means there will be some overlap between previous
        % and next graphs
        xlim = xlim-str2double(get(diff_h,'string'))+overlap;
...
xsamples = round(xlim/(1/mep.tmp.Hertz));
xdata = (xsamples)*(1/mep.tmp.Hertz);
% 
...
% 
% if size(mep.tmp.data,2) == numel(mep.data.channel_labels)
%     if ~isfield(mep,'data') && ~isfield(mep.data,'channel_colours')
%         mep.data.channel_colours = {'b','r','g'};
%     end
%     tmp_vis = cell(1,size(mep.tmp.data,2));
%     check_h = zeros(1,size(mep.tmp.data,2));
%     for i = 1 : size(mep.tmp.data,2)
%         check_h(i) = legend_ch(strcmpi(get(legend_ch,'String'),mep.data.channel_labels{i}));
%         tmp_vis{i} = get(get(check_h(i),'UserData'),'Visible');
%     end
%     for i = 1 : size(mep.tmp.data,2)
%         if i == 1 && ishold(axes_h); hold; end
%         
%         mep.tmp.ev_plot = [mep.data.channel_labels{i},'_plot'];
%         if numel(unique(mep.tmp.data(:,i))) == 2 && ...
%                 isfield(mep.data,mep.tmp.ev_plot)
%             mep.tmp.events = find(mep.tmp.data(:,i));
% 
%             xdata_ev = mep.data.(mep.tmp.ev_plot)(:,2)*(1/mep.tmp.Hertz);
%             ydata_ev = mep.data.(mep.tmp.ev_plot)(:,1)*max(ylim);
%             filt_ev = and(xdata_ev >= xdata(1),xdata_ev <= xdata(2));
% 
%             line_h = plot(axes_h,xdata_ev(filt_ev),...
%                 ydata_ev(filt_ev),...
%                 'color',mepPlotColours(mep.data.channel_labels{i}),...mep.data.channel_colours{i},...
%                 'Tag',mep.data.channel_labels{i},...
%                 'DisplayName',mep.data.channel_labels{i});
%             
%         else
%             
%             line_h = plot(axes_h,xdata(1):(1/mep.tmp.Hertz):xdata(2),...
%                 mep.tmp.data(xsamples(1):xsamples(2),i),...
%                 'color',mepPlotColours(mep.data.channel_labels{i}),...mep.data.channel_colours{i},...
%                 'Visible',tmp_vis{i});
%             
%         end
...
%     end
% else
%     plot(axes_h,xdata(1):(1/mep.tmp.Hertz):xdata(2),...
%         mep.tmp.data(xsamples(1):xsamples(2),:));
% end

% delete any number: condition information before adjusting the axes
