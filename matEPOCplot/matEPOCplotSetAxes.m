function matEPOCplotSetAxes(dop)
...
% if max(get(dop.fig.ax,'Xlim')) ~= max(xdata) || min(get(dop.fig.ax,'Xlim')) ~= min(xdata)
%     set(dop.fig.ax,'Xlim',[min(xdata) max(xdata)]);%,'TickDir','out');
% end

set(dop.fig.ax,'TickDir','out');
...
if ~isempty(strcmp(get(dop.fig.ch,'Type'),'uicontrol')) ...
        && ~isempty(strcmp(get(dop.fig.ch,'Tag'),'xlower'))
    xlim = get(dop.fig.ax,'Xlim');
    set(dop.fig.ch(and(strcmp(get(dop.fig.ch,'Type'),'uicontrol'),...
        strcmp(get(dop.fig.ch,'Tag'),'lower'))),'string',xlim(1));
    set(dop.fig.ch(and(strcmp(get(dop.fig.ch,'Type'),'uicontrol'),...
        strcmp(get(dop.fig.ch,'Tag'),'upper'))),'string',xlim(2));
    
    set(dop.fig.ch(and(strcmp(get(dop.fig.ch,'Type'),'uicontrol'),...
        strcmp(get(dop.fig.ch,'Tag'),'diff'))),'string',diff(xlim));
end
