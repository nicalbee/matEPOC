function fig = matEPOCplotComponents(fig_h,varargin)
% dopOSCCI3: matEPOCplotComponents
%
...
if ~xoff %&& ~isempty(varargin) && ~sum(strcmp(varargin,'epoch'))
    ...
    for i = 1 : numel(fig.xbut.pos.x)
        ...
        if ~isempty(varargin) && sum(strcmp(varargin,'epoch'))
            set(fig.xbut.h(i),'CallBack', @matEPOCplotEpochAxesAdjust);
        end
    end
    ...
    fig.xedit.default_data([1 3]) = [(1/data.tmp.Hertz) size(data.tmp.data,1)*(1/data.tmp.Hertz)];
    ...
    for i = 1 : numel(fig.xedit.tags)
        ...
        if ~isempty(varargin) && sum(strcmp(varargin,'epoch'))
            set(fig.xedit.h(i),'CallBack', @matEPOCplotEpochAxesAdjust);
        end
    end
end
%% Y axis adjustment
...
if ~yoff
    ...
    for i = 1 : numel(fig.ybut.text)
        ...
        fig.ybut.h(i) = uicontrol('parent',fig.h,'units','normalized',...
            ...
            'tag',fig.ybut.tags{i},...
            'CallBack',@matEPOCplotAxesAdjust,...@matEPOCplotYadjust,...
            'ToolTipString',fig.ybut.tips{i},...
            'position',fig.ybut.position(i,:));
        if ~isempty(varargin) && sum(strcmp(varargin,'epoch'))
            set(fig.ybut.h(i),'CallBack', @matEPOCplotEpochAxesAdjust);
        end
    end
    ...
    for i = 1 : numel(fig.yedit.tags)
        ...
        if ~isempty(varargin) && sum(strcmp(varargin,'epoch'))
            set(fig.yedit.h(i),'CallBack', @matEPOCplotEpochAxesAdjust);
        end
    end
end
%% offset adjust
